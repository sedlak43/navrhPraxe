...
#[ORM\Entity]
#[ORM\Table(name: 'zajezdy_student')]
class ZajezdyStudent
{
    #[ORM\Id]
    #[ORM\GeneratedValue]
    ...
    #[ORM\OneToMany(targetEntity: DatumyStudent::class, mappedBy: 'zajezd_student', cascade: ['persist', 'remove'], orphanRemoval: true)]
    private Collection $datumy_student;
    ...
    #[ORM\OneToMany(targetEntity: PricingItemStudent::class, mappedBy: 'zajezd_student', cascade: ['persist', 'remove'], orphanRemoval: true)]
    private Collection $pricingItems_student;
    ...
    {
        $this->datumy_student = new ArrayCollection();
        $this->pricingItems_student = new ArrayCollection();
    ...
    public function getDatumyStudent(): Collection
    {
        return $this->datumy_student;
    }
    ...
    public function addDatum(DatumyStudent $datum): static
    {
        if (!$this->datumy_student->contains($datum)) {
            $this->datumy_student[] = $datum;
            $datum->setZajezdStudent($this); // Set the owning side
        }

        return $this;
    }

    public function removeDatum(DatumyStudent $datum): static
    {
        if ($this->datumy_student->removeElement($datum)) {
            // Set the owning side to null
            if ($datum->getZajezdStudent() === $this) {
                $datum->setZajezdStudent(null);
            }
        }
    ...
    public function getPricingItemsStudent(): Collection
    {
        return $this->pricingItems_student;
    }

    public function setPricingItemsStudent(Collection $pricingItems): self
    {
        $this->pricingItems_student = $pricingItems;
        return $this;
    }


    public function addPricingItemStudent(PricingItemStudent $pricingItemStudent): self
    {
        if (!$this->pricingItems_student->contains($pricingItemStudent)) {
            $this->pricingItems_student[] = $pricingItemStudent;
            $pricingItemStudent->setZajezdStudent($this);  // Ensure the reverse relationship is set
        }
    ...
    public function removePricingItem(PricingItemStudent $pricingItemStudent): self
    {
        if ($this->pricingItems_student->removeElement($pricingItemStudent)) {
            if ($pricingItemStudent->getZajezdStudent() === $this) {
                $pricingItemStudent->setZajezdStudent(null);
            }
        }
